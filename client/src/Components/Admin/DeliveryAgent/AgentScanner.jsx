...
function AgentScanner() {
    ...
    const makeAttendance = async () => {
        try {
            const { data } = await axios.post(`/api/v1/orders/deliver/${formData}/${user.agentId}`);
            if (data.success) {
    ...
    return (
        <>
            <div className='w-full flex flex-col mt-14 min-h-screen'>
                <h2 className='mx-auto text-xl font-semibold mt-5'>Deliver Parcel</h2>
                ...
                <div className='flex justify-center space-x-4 -mt-[40vh]'>
                    ...
                    {result && <button onClick={makeAttendance} className='bg-[#8877f5] p-2 px-6 rounded-md text-sm'>Make Delivered</button>}
                </div>
